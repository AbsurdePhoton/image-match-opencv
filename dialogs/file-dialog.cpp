...
void PreviewFileDialog::OnCurrentChanged(const QString & path) // show thumbnail when a file is selected
{
    ...
    QString extension = selectedFilename.mid(selectedFilename.lastIndexOf("."), -1); // get file extension

    if (previewImageCommon.contains(extension, Qt::CaseInsensitive)) {
        QPixmap pixmap = QPixmap(path); // create image from path

        if (pixmap.isNull()) { // not an image for Qt ?
            // let's try to open image with OpenCV
            cv::Mat img = cv::imread(path.toUtf8().constData(), cv::IMREAD_COLOR);
            pixmap = Mat2QPixmap(img);
        }
